...
interface MWLoaderProps {
    /** Subtitle text shown below the main "Betöltés" heading */
    subtitle?: string;
}

/**
 * Unified full-page loader component for the Most Wanted application.
 * Clean, minimal design – logo, text, and a subtle progress bar.
 */
export default function MWLoader({ subtitle = 'Kérem várjon...' }: MWLoaderProps) {
    return (
        <div className="flex items-center justify-center h-screen bg-gradient-to-br from-[#1a1a1a] via-[#0f0f0f] to-black relative overflow-hidden">
            ...
            <div
                ...
                style={{ animation: 'mwGlow 10s ease-in-out infinite' }}
            />
            <div
                className="absolute bottom-[20%] right-[15%] w-[350px] h-[350px] bg-blue-500/[0.03] rounded-full blur-[140px] pointer-events-none"
                style={{ animation: 'mwGlow 10s ease-in-out infinite 5s' }}
            />

            {/* Centered content */}
            <div className="relative z-10 flex flex-col items-center" style={{ animation: 'mwFadeIn 0.6s ease-out' }}>
                ...
                <div className="w-48 h-[2px] bg-white/[0.06] rounded-full overflow-hidden mb-6">
                    <div
                        ...
                        style={{ animation: 'mwBar 1.5s linear infinite' }}
                    />
                </div>
