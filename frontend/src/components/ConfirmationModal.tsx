...
export default function ConfirmationModal({
  ...
  /** Bezárás animáció alatt a szülő ürítheti a message-et — a rögzített szöveget mutatjuk */
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- eredeti minta: csak isOpen váltás indítja a bezáró animációt
  }, [isOpen]);
  ...
  return (
    <div
      className={`fixed inset-0 z-[9999] flex items-center justify-center transition-all ${isAnimating || !isOpen ? 'pointer-events-none' : ''}`}
    >
      <div
      ...
      <div
        className={`relative w-full max-w-sm bg-[#1a1a1a] rounded-2xl border border-white/10 shadow-2xl overflow-hidden ${isAnimating || !isOpen ? 'animate-scale-out' : 'animate-scale-in'}`}
      >
        <div
          className={`p-5 border-b border-white/5 bg-gradient-to-r ${theme.headerGradient} to-transparent flex items-center justify-between`}
        >
          <div className="flex items-center gap-3 min-w-0">
            <div className={`p-2 rounded-lg shrink-0 ${theme.iconWrapper}`}>
              <FiAlertTriangle className={`w-5 h-5 ${theme.icon}`} />
            </div>
            <h3 className="text-lg font-bold text-white truncate">{displayTitle}</h3>
          </div>
          <button
            type="button"
            onClick={onCancel}
        ...
        <div className="p-7">
          <p className="text-gray-300 leading-relaxed text-base whitespace-pre-wrap">{displayMessage}</p>
        </div>
