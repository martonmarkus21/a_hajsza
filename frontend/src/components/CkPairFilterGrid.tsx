...
interface CkPairFilterGridProps {
  pairs: Pair[];
  value: string;
...
/** Pár szűrő — CkDropdownSelect-szerű animáció; nyitott triggeren kontrasztos szöveg/ikon. */
export default function CkPairFilterGrid({ pairs, value, onChange, className = '' }: CkPairFilterGridProps) {
  ...
  const pairBadgeClass = (ck: boolean, captured: boolean, compact: boolean) => {
    const base = compact
    ...
    return `${base} ${ck ? 'bg-orange-500' : 'bg-[#2a2a2a]'}`;
  };
          ...
          <div className="grid grid-cols-4 sm:grid-cols-5 gap-2">
            ...
            {sorted.map((p) => {
              ...
              return (
                ...
                >
                  <span className={pairBadgeClass(!!p.celkereszt, !!p.captured, false)}>{p.assignedNumber}</span>
                </button>
              );
            })}
          </div>
  ...
  return (
    <div className={`relative ${className}`}>
      <button
        ...
        onClick={() => setOpen((o) => !o)}
        className={`ck-btn ck-filter-trigger !flex h-10 w-full flex-row items-center justify-between gap-2 px-3 py-0 leading-none text-left ${
          open
            ? 'ck-btn-primary !text-white shadow-none [&_.ck-pair-chevron]:text-white'
      ...
      >
            ...
            <span
              className={
                open
                  ...
                  : pairBadgeClass(!!selectedPair.celkereszt, !!selectedPair.captured, true)
              }
            >
              {selectedPair.assignedNumber}
            </span>
        ...
        <span className="flex h-10 w-9 shrink-0 items-center justify-center">
          <FiChevronDown
            className={`ck-pair-chevron h-4 w-4 transition-transform duration-200 ease-out ${open ? 'rotate-180' : ''}`}
            aria-hidden
          />
        </span>
      </button>
