...
interface CkDateTimePickerProps {
  label: string;
  value: string;
...
export default function CkDateTimePicker({
  label,
  value,
  onChange,
  className = '',
  ...
  return (
    <div className={`space-y-1.5 ${className}`}>
      ...
      <button
        ...
        className={`ck-btn ck-filter-trigger !flex h-11 w-full flex-row items-center justify-between gap-2 px-3 py-0.5 leading-none [&_.ck-dt-row]:items-center ${
          open
            ? 'ck-btn-primary !text-white shadow-none [&_.ck-dt-chevron]:text-white'
            : 'ck-btn-secondary text-gray-400 hover:text-white'
        }`}
      >
        <span className="ck-dt-row flex min-w-0 flex-1 items-center gap-2 overflow-hidden">
          <span
        ...
        <span className="flex h-11 w-9 shrink-0 items-center justify-center">
          <FiChevronDown
            className={`ck-dt-chevron h-4 w-4 transition-transform duration-200 ease-out ${open ? 'rotate-180' : ''}`}
            aria-hidden
          />
        </span>
      </button>
