...
export interface MwDropdownOption {
  value: string;
  label: string;
}

interface MwDropdownSelectProps {
  value: string;
  onChange: (value: string) => void;
  options: MwDropdownOption[];
  /** Képernyőolvasóhoz */
  ariaLabel: string;
  className?: string;
...
/** Egyedi legördülő – trigger: mw-btn (ugyanaz a magasság, mint a szűrőgombok), portálon. */
export default function MwDropdownSelect({
  value,
  onChange,
  options,
  ariaLabel,
  ...
  return (
    <div className={className}>
      <button
        ...
        onClick={() => setOpen((o) => !o)}
        className={`mw-btn w-full min-h-0 !justify-between text-left items-center ${
          open ? 'mw-btn-primary' : 'mw-btn-secondary text-gray-400 hover:text-white'
        }`}
      >
